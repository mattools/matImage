...
% lineXmin = createLine([xmin ymin], [xmin ymax]);
% lineXmax = createLine([xmax ymin], [xmax ymax]);
% lineYmin = createLine([xmin ymin], [xmax ymin]);
% lineYmax = createLine([xmin ymax], [xmax ymax]);
% 
% % gather lines
% lines = [lineXmin;lineXmax;lineYmin;lineYmax];
...
for e=1:size(edges, 1)    
    ...
    if in1 && in2
        edges2 = [edges2; hashNodes(edges(e, :))'];
    ...
    if ~in1
        nodes2 = [nodes2; edge([1 2])];
        indN1 = size(nodes2, 1);
        %hashNodes(edges(e,1)) = indN1;
    else
    ...
    if ~in2
        nodes2 = [nodes2; edge([3 4])];
        indN2 = size(nodes2, 1);
        %hashNodes(edges(e,2)) = indN2;
    else
        indN2 = hashNodes(edges(e, 2));
    end
    
    % add clipped edge to the list
    edges2 = [edges2; indN1 indN2];
    hashEdges(e) = size(edges2, 1);
end
...
for f=1:length(faces)
    ...
    if sum(ind)>0
        nodes2 = [nodes2; corners(ind, :)];
        face2 = [face2; size(nodes2, 1)];
    end
    ...
    [faceNodes I] = angleSort(faceNodes, centroid(faceNodes));
    
    % add current face to list of faces
    faces2 = {faces2{:}, face2(I)'};
end
