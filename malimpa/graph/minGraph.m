function varargout = minGraph(nodes, edges)
%MINGRAPH merge all edges between 2-degrees nodes.
%
%   TODO: Temporary version used for debugging.
%
%   [NODES2 EDGES2] = minGraph(NODES, EDGES)
...
p = 1;
%while length(getNeighbourEdges(p, edges))<3
...
for b=1:length(neighbours)
    ...
    if edge(1)==p
        branch(Nb, 1:2) = [p edge(2)];
    else
        branch(Nb, 1:2) = [p edge(1)];
    end
end
...
%         node, nodes(node, 1), nodes(node, 2)));
    ...
    if ~isempty(index)
        ...
        for n=1:length(neighbours)
            ...
            if edge(1)~=pNode && edge(2)~=pNode
                ...
                Nb = Nb+1;
                if edge(1)==node
                    branch(Nb, 1:2) = [node edge(2)];
                else
                    branch(Nb, 1:2) = [node edge(1)];
                end
            end
        end
    end
    ...
    %disp('add new Edge');
    Ne = Ne+1;
    Sedges(Ne,1:2) = [find(Mpoints==branch(b,1)), index];
end
...
% % return edges around a given node
